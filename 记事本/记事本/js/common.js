...
function addMemoire() {
	// 进入
	$("#addMemoire").click(function (e) {
		$("edit-page").css("display", "block").animate({
			"left": "0%"
		},300);
		// 如果该事件是通过用户手动点击的（修改编辑时是通过trigger触发的）
		if($(e.currentTarget).is("#addMemoire")) {
			$("#memoireContent").prop("readonly", false);
		}
	});
	// 返回
